        ...
        <div class="atestado-content">
            <div class="atestado-text">
                <p style="text-indent: 5mm; margin-bottom: 2mm;">
                    Atesto para os devidos fins que o(a) paciente <span class="paciente-nome">{{ $consulta->paciente->nome }}</span>, 
                    NIF: {{ $consulta->paciente->nif }},
                    @if($consulta->paciente->data_nascimento)
                        nascido(a) em {{ \Carbon\Carbon::parse($consulta->paciente->data_nascimento)->format('d/m/Y') }},
                    @endif
                    foi atendido(a) nesta unidade de saúde em {{ \Carbon\Carbon::parse($consulta->data_consulta)->format('d/m/Y') }}.
                </p>
                
                @if($tipo_atestado === 'doenca' && $dias_afastamento)
                <p style="text-indent: 5mm; margin-bottom: 2mm;">
                    Em virtude do quadro clínico apresentado, recomendo afastamento das atividades por <strong>{{ $dias_afastamento }} dia(s)</strong>, 
                    a partir de {{ \Carbon\Carbon::parse($consulta->data_consulta)->format('d/m/Y') }}.
                </p>
                @elseif($tipo_atestado === 'saude')
                <p style="text-indent: 5mm; margin-bottom: 2mm;">
                ...
                @elseif($tipo_atestado === 'comparecimento')
                <p style="text-indent: 5mm; margin-bottom: 2mm;">
                    Atesto que o(a) paciente compareceu a esta unidade de saúde para atendimento médico em {{ \Carbon\Carbon::parse($consulta->data_consulta)->format('d/m/Y') }}.
                </p>
                @endif
