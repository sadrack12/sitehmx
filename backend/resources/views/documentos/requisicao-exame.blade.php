...
<head>
    ...
    <title>Requisição de Exame</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="documento-container">
        ...
        <div class="documento-title">
            <h1>REQUISIÇÃO DE EXAME LABORATORIAL</h1>
        ...
        <div class="info-section">
            ...
            <div class="info-item">
                ...
                <span class="info-value">{{ \Carbon\Carbon::parse($solicitacao->data_solicitacao)->format('d/m/Y') }}</span>
            </div>
            @if($solicitacao->data_prevista_realizacao)
            <div class="info-item">
                <span class="info-label">Data Prevista:</span>
                <span class="info-value">{{ \Carbon\Carbon::parse($solicitacao->data_prevista_realizacao)->format('d/m/Y') }}</span>
            </div>
            @endif
            @if($solicitacao->medicoSolicitante)
            <div class="info-item">
                <span class="info-label">Médico Solicitante:</span>
                <span class="info-value">{{ $solicitacao->medicoSolicitante->nome }}@if($solicitacao->medicoSolicitante->crm) - Nº Ordem: {{ $solicitacao->medicoSolicitante->crm }}@endif</span>
            </div>
            @endif
            @if($solicitacao->urgente)
            <div class="info-item">
                <span class="info-label">Prioridade:</span>
                <span class="info-value"><span class="urgente-badge">URGENTE</span></span>
            </div>
            @endif
        </div>
        ...
        <div class="info-section">
            ...
            <div class="exame-box">
                ...
                @endif
                @if($solicitacao->exame->descricao)
                <div class="exame-codigo" style="margin-top: 0.5mm;">{{ $solicitacao->exame->descricao }}</div>
                @endif
            </div>
        </div>
        ...
        <div class="info-section">
            ...
            <div class="observacoes-box">
                <div class="observacoes-text">{{ $solicitacao->observacoes }}</div>
            </div>
        </div>
        @endif
        ...
        <div class="assinatura-section">
            ...
            <div class="assinatura-text">
                @if($solicitacao->medicoSolicitante)
                    {{ $solicitacao->medicoSolicitante->nome }}@if($solicitacao->medicoSolicitante->crm) - Nº Ordem: {{ $solicitacao->medicoSolicitante->crm }}@endif
                @else
                    Médico Solicitante
                @endif
            </div>
