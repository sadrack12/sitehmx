...
import { Plus, Edit, Trash2, Loader2, Stethoscope, Eye, Calendar, GraduationCap } from 'lucide-react'
import { toast } from '@/components/ui/Toast'
import ConfirmDialog from '@/components/ui/ConfirmDialog'
import Select from '@/components/ui/Select'
import MedicoDetailsModal from '@/components/gestao/admin/MedicoDetailsModal'
import AgendaMedicoModal from '@/components/gestao/configuracoes/AgendaMedicoModal'
...
export default function MedicosPage() {
  ...
  const [especialidades, setEspecialidades] = useState<{ id: number; nome: string; ativa: boolean }[]>([])
  ...
  const [agendaMedico, setAgendaMedico] = useState<Medico | null>(null)
  ...
  const fetchEspecialidades = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const especialidadesAtivas = (data.data || []).filter((e: any) => e.ativa)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await response.json()
      const medicoSalvo = data.data || data
      
      toast.success(editingMedico ? 'Médico atualizado com sucesso!' : 'Médico criado com sucesso!')
      await fetchMedicos()
      resetForm()
      
      // Após criar médico, perguntar se quer definir agenda
      if (!editingMedico) {
        setTimeout(() => {
          if (window.confirm('Médico criado com sucesso! Deseja definir a agenda de trabalho agora?')) {
            setAgendaMedico(medicoSalvo)
            setShowAgenda(true)
          }
        }, 500)
      }
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {especialidades.length > 0 ? (
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        ...
                        <a href="/gestao/configuracoes/especialidades" className="text-green-600 hover:underline">
                          Especialidades
                        </a>
                      </p>
                    </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">Email de contato do médico</p>
                </div>
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">Telefone de contato</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2 pb-2 border-b border-gray-200">
                ...
                <h3 className="text-base font-semibold text-gray-900">Acesso ao Sistema</h3>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div className="flex-1">
                    <label htmlFor="criar_login" className="text-sm font-medium text-gray-700 cursor-pointer">
                      Criar perfil de acesso (login)
                    </label>
                    <p className="text-xs text-gray-500 mt-1">
                      Permite que o médico faça login no sistema para acessar seu painel
                    </p>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 pt-4 border-t border-gray-300">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-1 focus:ring-green-500 focus:border-green-500 outline-none text-sm bg-white"
                        placeholder="email@exemplo.com"
                      />
                      <p className="text-xs text-gray-500 mt-1">Email usado para fazer login no sistema</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-1 focus:ring-green-500 focus:border-green-500 outline-none text-sm bg-white"
                        placeholder={editingMedico ? "Deixe em branco para manter" : "Mínimo 6 caracteres"}
                      />
                      {editingMedico ? (
